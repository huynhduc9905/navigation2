...
bool ExclusionZone::configure()
{
  ...
  if (visualize_) {
    zone_pub_ = node->create_publisher<geometry_msgs::msg::PolygonStamped>(
      zone_name_, rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
  }
...
bool ExclusionZone::getParameters()
{
  ...
  enabled_ = nav2::declare_or_get_parameter(node, zone_name_ + ".enabled", false);
  visualize_ = nav2::declare_or_get_parameter(node, zone_name_ + ".visualize", false);

  // Frame the zone is anchored to. Empty -> static zone in the robot base frame.
  frame_id_ = nav2::declare_or_get_parameter(node, 
    zone_name_ + ".frame_id", base_frame_id_);
  if (frame_id_.empty()) {
    frame_id_ = base_frame_id_;
  }

  frame_hold_timeout_ = nav2::declare_or_get_parameter(node, 
    zone_name_ + ".frame_hold_timeout", 0.0);
  // Optional height band (in base frame). Unbounded by default so 2D sources are covered.
  min_height_ = nav2::declare_or_get_parameter(node,
    zone_name_ + ".min_height", -std::numeric_limits<double>::max());
  max_height_ = nav2::declare_or_get_parameter(node,
    zone_name_ + ".max_height", std::numeric_limits<double>::max());

  const std::string type = nav2::declare_or_get_parameter(node,
    zone_name_ + ".type", std::string("polygon"));

  if (type == "circle") {
    is_circle_ = true;
    radius_ = nav2::declare_or_get_parameter(node, zone_name_ + ".radius", -1.0);
    if (radius_ <= 0.0) {
      RCLCPP_ERROR(
    ...
    const std::string points_str = nav2::declare_or_get_parameter(node,
